    ...
    <header className="sticky top-0 z-10 bg-white shadow-sm">
      <div className="max-w-6xl mx-auto px-4 flex h-16 items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-4">
          <NavLink className={({isActive}) => `text-sm ${isActive ? 'text-brand font-medium' : 'text-gray-600 hover:text-gray-900'}`} to="/dashboard">Главная</NavLink>
          <NavLink className={({isActive}) => `text-sm ${isActive ? 'text-brand font-medium' : 'text-gray-600 hover:text-gray-900'}`} to="/schedule">Расписание</NavLink>
          <NavLink className={({isActive}) => `text-sm ${isActive ? 'text-brand font-medium' : 'text-gray-600 hover:text-gray-900'}`} to="/materials">Материалы</NavLink>
          {profile?.role === 'admin' && (
            <NavLink className={({isActive}) => `text-sm ${isActive ? 'text-brand font-medium' : 'text-gray-600 hover:text-gray-900'}`} to="/admin">Админ</NavLink>
          )}
          {profile && (
            <button
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 shadow-sm">
          <div className="max-w-6xl mx-auto px-4 py-2 space-y-1">
            <NavLink 
              className={({isActive}) => `block py-2 px-3 rounded-lg ${isActive ? 'bg-brand/10 text-brand font-medium' : 'text-gray-600 hover:bg-gray-50'}`}
              to="/dashboard"
